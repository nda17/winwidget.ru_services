...
Канонический backlog хранится в `DOCUMENTATION/backlog.md` вместе с
...
Read-only аудит production API от 27.08.2026 подтверждает активный не тестовый
магазин, включённую фискализацию, оплату банковской картой и наличие успешных
платежей с зарегистрированными чеками. Это не доказывает работу текущей ревизии
при новом первоначальном и рекуррентном списании. До появления клиентов
требуется внешняя проверка production-контура:

- в личном кабинете ЮKassa проверить договорное включение автоплатежей и
  актуальные URL/события HTTP-уведомлений: для Basic Auth эти настройки не
  доступны через API;
...
- подтвердить доставку чека покупателю, теги 1125, 1187 и 1008, OFD links,
  обработку `receipt.canceled`, ошибок регистрации и корректирующего чека;
...
### P1 — аудит online consultant ранее деактивированных пользователей

Провести read-only аудит активных online consultant у пользователей,
деактивированных до исправления общей деактивации. Массовую коррекцию выполнять
только после отдельного dry-run и точной проверки target records.
