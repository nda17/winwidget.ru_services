...
if [[ "$DEPLOY_TARGET" == 'platform' ]]; then
  case "$PLATFORM_ACTION:$PLATFORM_CONFIRMATION" in
    status:) ;;
    prepare-billing-readiness:'PREPARE PLATFORM BILLING READINESS') ;;
    prepare:'PREPARE PLATFORM OWNERSHIP') ;;
    cutover:'CUTOVER PLATFORM OWNERSHIP'|forward-recovery:'CUTOVER PLATFORM OWNERSHIP') ;;
    abort:'ABORT PLATFORM CUTOVER') ;;
    *)
      echo 'Invalid Platform lifecycle action or confirmation.' >&2
      exit 1
      ;;
  esac
  case "$PLATFORM_ACTION" in
    prepare-billing-readiness|prepare|cutover|forward-recovery)
      [[ "$PLATFORM_ENV_EXPECTED_SHA256" =~ ^[0-9a-f]{64}$ &&
        ...
        echo 'Platform mutation requires exact env and signed frontend evidence pins.' >&2
        exit 1
      }
      ;;
    abort)
      [[ "$PLATFORM_ENV_EXPECTED_SHA256" =~ ^[0-9a-f]{64}$ &&
        -z "$PLATFORM_FRONTEND_REVISION$PLATFORM_FRONTEND_RUNTIME_CHALLENGE$PLATFORM_FRONTEND_ORIGIN$PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256" ]] || exit 1
      ;;
    status)
      [[ -z "$PLATFORM_ENV_EXPECTED_SHA256$PLATFORM_FRONTEND_REVISION$PLATFORM_FRONTEND_RUNTIME_CHALLENGE$PLATFORM_FRONTEND_ORIGIN$PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256" ]] || exit 1
      ;;
  esac
  [[ -z "$PLATFORM_CLEANUP_MIGRATION_SHA256$PLATFORM_CLEANUP_COMPOSE_SHA256$PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256$PLATFORM_CLEANUP_SOAK_SECONDS" ]] || {
    echo 'Platform cleanup inputs require the platform-cleanup target.' >&2
    exit 1
  }
elif [[ "$DEPLOY_TARGET" == 'platform-cleanup' ]]; then
  case "$PLATFORM_ACTION:$PLATFORM_CONFIRMATION" in
    status:|inventory:)
      [[ -z "$PLATFORM_ENV_EXPECTED_SHA256$PLATFORM_FRONTEND_REVISION$PLATFORM_FRONTEND_RUNTIME_CHALLENGE$PLATFORM_FRONTEND_ORIGIN$PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256$PLATFORM_CLEANUP_MIGRATION_SHA256$PLATFORM_CLEANUP_COMPOSE_SHA256$PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256$PLATFORM_CLEANUP_SOAK_SECONDS" ]] || exit 1
      ;;
    stage:'DROP LEGACY PLATFORM CORE SOURCE'|seal:'DROP LEGACY PLATFORM CORE SOURCE'|run:'DROP LEGACY PLATFORM CORE SOURCE'|verify:'DROP LEGACY PLATFORM CORE SOURCE'|forward-recovery:'DROP LEGACY PLATFORM CORE SOURCE')
      [[ "$PLATFORM_ENV_EXPECTED_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_FRONTEND_REVISION" =~ ^[0-9a-f]{40}$ &&
        "$PLATFORM_FRONTEND_RUNTIME_CHALLENGE" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_FRONTEND_ORIGIN" =~ ^https://[a-z0-9.-]+(:[1-9][0-9]{0,4})?$ &&
        ...
        "$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_CLEANUP_MIGRATION_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_CLEANUP_COMPOSE_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256" =~ ^[0-9a-f]{64}$ &&
        "$PLATFORM_CLEANUP_SOAK_SECONDS" =~ ^[1-9][0-9]{2,6}$ ]] || {
        echo 'Platform cleanup mutation requires exact env, migration, Compose, first-COMPLETE and signed frontend evidence pins.' >&2
        exit 1
      }
      ;;
    *)
      echo 'Invalid Platform Core cleanup action or confirmation.' >&2
      exit 1
      ;;
  esac
elif [[ "$PLATFORM_ACTION" != 'status' || -n "$PLATFORM_CONFIRMATION" ||
  -n "$PLATFORM_ENV_EXPECTED_SHA256$PLATFORM_FRONTEND_REVISION$PLATFORM_FRONTEND_RUNTIME_CHALLENGE$PLATFORM_FRONTEND_ORIGIN$PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256$PLATFORM_CLEANUP_MIGRATION_SHA256$PLATFORM_CLEANUP_COMPOSE_SHA256$PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256$PLATFORM_CLEANUP_SOAK_SECONDS" ]]; then
  echo 'Platform lifecycle inputs require the platform target.' >&2
  exit 1
fi
if [[ "$DEPLOY_TARGET" == 'support' ]]; then
  case "$SUPPORT_ACTION:$SUPPORT_CONFIRMATION" in
    status:) ;;
    prepare:'PREPARE SUPPORT OWNERSHIP') ;;
    cutover:'CUTOVER SUPPORT OWNERSHIP') ;;
    cleanup:'DROP LEGACY SUPPORT CORE SOURCE') ;;
    *)
      echo 'Invalid Support lifecycle action or confirmation.' >&2
      exit 1
      ;;
  esac
  if [[ "$SUPPORT_ACTION" == 'cleanup' ]]; then
  ...
  elif [[ -n "$SUPPORT_CLEANUP_MIGRATION_SHA256$SUPPORT_CLEANUP_BACKUP_SHA256$SUPPORT_CLEANUP_RESTORE_EVIDENCE_SHA256" ]]; then
    echo 'Support cleanup evidence is accepted only by the cleanup action.' >&2
    exit 1
  fi
elif [[ "$SUPPORT_ACTION" != 'status' || -n "$SUPPORT_CONFIRMATION" ||
  -n "$SUPPORT_CLEANUP_MIGRATION_SHA256$SUPPORT_CLEANUP_BACKUP_SHA256$SUPPORT_CLEANUP_RESTORE_EVIDENCE_SHA256" ]]; then
  echo 'Support lifecycle inputs require the support target.' >&2
  exit 1
fi
if [[ "$DEPLOY_TARGET" == 'operations' ]]; then
  case "$OPERATIONS_ACTION:$OPERATIONS_CONFIRMATION" in
    status:)
      [[ -z "$OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION" ]] || exit 1
      ;;
    cutover:'CUT OVER OPERATIONS FORWARD ONLY')
      [[ -z "$OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION" ||
        "$OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION" == 'DROP PLATFORM CORE SOURCE DURING OPERATIONS CUTOVER' ]] || exit 1
      ;;
    *)
      echo 'Invalid Operations lifecycle action or confirmation.' >&2
      exit 1
      ;;
  esac
elif [[ "$OPERATIONS_ACTION" != 'status' || -n "$OPERATIONS_CONFIRMATION" ||
  -n "$OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION" ]]; then
  echo 'Operations lifecycle inputs require the operations target.' >&2
  exit 1
fi
...
APP_ROOT='$APP_ROOT' AUTOMATIC_PROD_PUSH='$AUTOMATIC_PROD_PUSH' EXPECTED_REVISION='$DEPLOY_REVISION' DEPLOY_TARGET='$DEPLOY_TARGET' WIDGETS_CUTOVER_CONFIRMATION='$WIDGETS_CUTOVER_CONFIRMATION' BILLING_ACTION='$BILLING_ACTION' BILLING_CONFIRMATION='$BILLING_CONFIRMATION' BILLING_CLEANUP_ACTION='$BILLING_CLEANUP_ACTION' BILLING_CLEANUP_CONFIRMATION='$BILLING_CLEANUP_CONFIRMATION' IDENTITY_ACTION='$IDENTITY_ACTION' IDENTITY_CONFIRMATION='$IDENTITY_CONFIRMATION' IDENTITY_ENV_EXPECTED_SHA256='$IDENTITY_ENV_EXPECTED_SHA256' IDENTITY_ENV_EXPORT_ID='$IDENTITY_ENV_EXPORT_ID' IDENTITY_CLEANUP_ACTION='$IDENTITY_CLEANUP_ACTION' IDENTITY_CLEANUP_CONFIRMATION='$IDENTITY_CLEANUP_CONFIRMATION' IDENTITY_CORE_CLEANUP_MIGRATION_SHA256='$IDENTITY_CLEANUP_MIGRATION_SHA256' PLATFORM_ACTION='$PLATFORM_ACTION' PLATFORM_CONFIRMATION='$PLATFORM_CONFIRMATION' PLATFORM_ENV_EXPECTED_SHA256='$PLATFORM_ENV_EXPECTED_SHA256' PLATFORM_FRONTEND_REVISION='$PLATFORM_FRONTEND_REVISION' PLATFORM_FRONTEND_RUNTIME_CHALLENGE='$PLATFORM_FRONTEND_RUNTIME_CHALLENGE' PLATFORM_FRONTEND_ORIGIN='$PLATFORM_FRONTEND_ORIGIN' PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256='$PLATFORM_FRONTEND_EXPECTED_ATTESTATION_SHA256' PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256='$PLATFORM_FRONTEND_EXPECTED_SIGNATURE_SHA256' PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256='$PLATFORM_FRONTEND_TRUSTED_PUBLIC_KEY_SHA256' PLATFORM_CLEANUP_MIGRATION_SHA256='$PLATFORM_CLEANUP_MIGRATION_SHA256' PLATFORM_CLEANUP_COMPOSE_SHA256='$PLATFORM_CLEANUP_COMPOSE_SHA256' PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256='$PLATFORM_CLEANUP_FIRST_COMPLETE_PROOF_SHA256' PLATFORM_CLEANUP_SOAK_SECONDS='$PLATFORM_CLEANUP_SOAK_SECONDS' SUPPORT_ACTION='$SUPPORT_ACTION' SUPPORT_CONFIRMATION='$SUPPORT_CONFIRMATION' SUPPORT_CLEANUP_MIGRATION_SHA256='$SUPPORT_CLEANUP_MIGRATION_SHA256' SUPPORT_CLEANUP_BACKUP_SHA256='$SUPPORT_CLEANUP_BACKUP_SHA256' SUPPORT_CLEANUP_RESTORE_EVIDENCE_SHA256='$SUPPORT_CLEANUP_RESTORE_EVIDENCE_SHA256' OPERATIONS_ACTION='$OPERATIONS_ACTION' OPERATIONS_CONFIRMATION='$OPERATIONS_CONFIRMATION' OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION='$OPERATIONS_PLATFORM_TERMINAL_CONFIRMATION' bash \"\$controller\"" |
  require_completion_receipt
trap - EXIT
cleanup_remote_controller
