...
const pinnedInfraRevision =
	'b96e9acd876087edf2a4a37aa1bcf2efdfe77d99';
...
if (
	...
	throw new Error('CRM reminders release must use one exact reviewed infra SHA');
}
// Activate the compatible reader, dedicated reminder process and Sales API.
// Preserve immutable images, all database state and neighboring processes.
for (const [job, scope] of [['deploy-production', 'crm-reminders-activate']]) {
	...
	if (!block || !block.includes('release_scope: ' + scope) ||
		...
		!block.includes("expected_service_env_sha256: '910c80c75219678eb94daf8e6cabc504f83c1e5fa5e6a1d00feba38edad430ff'") ||
		!block.includes("expected_crm_reminders_baseline_sha256: '73f379bf3413a7a14fcb669d9603c44381680ee3f72722ee18687d79c707fba2'")) {
		throw new Error('CRM reminders release must pin the Gateway revision, owner env and fresh exact baseline');
	}
}
