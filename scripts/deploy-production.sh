...
while IFS= read -r key; do
	[[ -n "$key" ]] || continue
	case "$key" in
	APP_REVISION | APP_VERSION | MAINTENANCE_IMAGE | MAINTENANCE_REVISION | DATABASE_RESTORE_IMAGE | DATABASE_RESTORE_REVISION | NOTIFICATION_DELIVERY_IMAGE | NOTIFICATION_DELIVERY_REVISION | CAMPAIGNS_IMAGE | CAMPAIGNS_REVISION | REPORTING_IMAGE | REPORTING_REVISION | WIDGETS_IMAGE | WIDGETS_REVISION | BILLING_IMAGE | BILLING_REVISION | IDENTITY_IMAGE | IDENTITY_REVISION | PLATFORM_IMAGE | PLATFORM_REVISION | SUPPORT_IMAGE | SUPPORT_REVISION)
			continue
			;;
	esac
