...
export class WidgetService {
	...
	async getPublicConfig(
		...
		const bonuses = (config.bonuses || []).map((b: any, i: number) => ({
			text: b.name,
			isInRaffle: b.active,
			order: i,
			probability: b.probability ?? 1,
			color: b.color || null,
			neverWin: b.neverWin || false
		}));
		...
		return {
			...
			headline: config.title || 'Крутите колесо!',
			subheadline:
				config.subtitle ||
				(() => {
			...
			autoOpenSeconds: config.autoOpenDelay || null,
			privacyPolicyUrl: config.privacyUrl || null,
