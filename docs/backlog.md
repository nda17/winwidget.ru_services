...
Новые сделки назначаются создателю; приглашение содержит email, роль и отделы,
имя сотрудника читается из Identity profile.

- Добавить профиль сотрудника в рабочем пространстве: имя, фамилия,
  необязательное отчество при приглашении и редактировании. Владение данными
  и права определить в CRM Access, не перезаписывать глобальный профиль
  существующего аккаунта Identity через CRM-администратора. Сохранить доступ
  прежним участникам без заполненного ФИО и безопасное отображение имени/email.
- Добавить серверный scoped directory доступных ответственных с поиском и
  пагинацией; отображать имя, сохранять subject/membership binding.
  Pending/отключённые/чужие сотрудники не назначаются. Не раскрывать
  административный справочник MANAGER/TEAM_LEAD; отдельно проверить владельца,
  который не занимает строку обычного CRM-member, и отзыв membership.
