...
уведомления. До реализации закрепить матрицу управления правилами и каналами,
ограничения частоты/получателей и семантику READ_ONLY; уведомления не должны
менять состояние подписки или обходить независимость Widgets.
...
### P1 — в заключительном этапе MVP подключить резервные копии четырёх БД CRM
...
В текущем коде отдельные backup-роли CRM предусмотрены, но Operations
`DATABASE_BACKUP_TARGETS`, карты URL/БД и env `maintenance-worker` содержат
только девять прежних целей. Наличие роли и успешная проверка доступа не
доказывают создание архива, расписание или доставку.

- Добавить `crm-access`, `crm-customers`, `crm-sales`, `crm-intake` как четыре
  независимые service-owned цели: точные БД/schema, least-privilege backup
  principals и worker-only подключения. Не менять поведение девяти прежних
  целей и не выдавать backup credentials API или бизнес-workers.
- Использовать текущие durable задания Operations с уникальным ключом периода,
  transactional Outbox и at-least-once RabbitMQ; dump выполняет только
  `maintenance-worker`. До включения новых целей устранить описанный ниже риск
  потери durable trigger при redelivery занятого lease и проверить crash/restart.
- Проверить четыре реальных архива и подтверждения Telegram-доставки в уже
  настроенный канал; не подменять это только health-check или правами на БД.
  Синхронизировать полные production env локально и на VPS без вывода секретов.
- Сохранить проверку целостности и происхождения артефактов. Текущий отдельный
  `.provenance.json` — подписанные Ed25519 метаданные, а не второй dump и не
  шифрование. Отделить список подписываемых backup от разрешённых restore
  targets, если добавляются подписи CRM: это не должно автоматически разрешать
  разрушительное восстановление CRM или снимать restore release gate.
- Учитывать ограничения размера Telegram и существующие требования retention;
  не создавать лишние постоянные локальные копии и не очищать рабочие БД.
