...
### P0 — завершить production browser-проверку продуктового сайта
...
- отдельные редакторы главной, Widgets, WinCRM и SEO в «Контенте», включая
  сохранение `ecosystem`/`crmProduct` из уже открытого старого редактора;
...
- сохранность текущего контента Widgets, query/UTM и старых точных
  `/#tools`, `/#pricing`, `/#faq` ссылок после browser hydration;
