...
- Завершить отдельную активацию оплаты: три полных owner/canonical env уже
  синхронизированы, но работающие процессы ещё сохраняют выключенные флаги.
  Обновление существующего GitHub CI/CD secret `BACKEND_PRODUCTION_ENV_SHA256`
  остановлено автоматической проверкой разрешений и требует отдельного
  подтверждения пользователя на передачу SHA-256 в тот же репозиторий.
  Не обходить запрет другим способом передачи и не запускать деплой с прежней
  контрольной суммой. После разрешения — свежий baseline и config-only
  `crm-commerce-activate`, затем проверка фактических runtime flags.
- По явному подтверждению пользователя 07.09.2026 включить платную WinCRM
  в текущем релизе: согласованно открыть Access/Billing и frontend CRM/Widgets,
  сохранив независимость оплат Widgets. Активация настроек не подтверждает
  внешние списания/фискализацию; реальные проверки ниже остаются с участием
  пользователя. До перезапуска проверить существующие CRM orders/renewals,
  выполнить двустороннюю синхронизацию owner env и узкий activation rollout;
  обычный code-only `crm-upgrade` не разрешает менять env.
...
### P1 — поиск компании/ИП по ИНН и сохранение реквизитов в текущем MVP
...
- Выпустить Customers-owned nullable реквизиты и v2 CRUD/экспорт, сохранив
  прежние v1 ответы, receipt/hash replay и ручной ввод. Не объединять компании
  автоматически и не записывать реквизиты в поле заметок.
- Подключить fixed-origin server-side lookup с текущей авторизацией до/после
  запроса, ограничением размера/времени/частоты, без ключа в браузере и без
  RabbitMQ для быстрого read-only запроса. Проверка checksum ИНН относится
  только к lookup; прежнюю валидацию ручного ввода не ужесточать.
- UI: явный поиск, проверка найденной головной организации/ИП, подтверждение
  заполнения и обычное сохранение. Не перезаписывать заполненные поля молча;
  сбрасывать устаревший результат при смене ИНН/сессии/workspace/прав.
- До production-активации получить пользовательский API-ключ в игнорируемом
  `deploy/backend/secrets/dadata-api-key`, синхронизировать owner env по
  принятой двусторонней процедуре и проверить реальные ответы. Отсутствие
  ключа, квота/ошибка поставщика не блокируют ручное создание компании.
- Подтвердить PG18 v1/v2 совместимость, мобильную/тёмную форму и JSON/CSV с
  реквизитами; после выпуска дополнить обе пользовательские PDF-инструкции.
