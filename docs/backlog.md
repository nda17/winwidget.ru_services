...
Подтвердить PostgreSQL 18 CI новых команд/ledger и выпуск миграции Billing
`20260910120000_add_crm_admin_day_grants`; выполнить production browser-проверки
обеих ролей. Сохранить исходный receipt при timeout; отсутствие GET
...
### P1 — завершить browser-проверку поиска компании/ИП по ИНН

Пользователь подтвердил 07.09.2026 включение автозаполнения в этот релиз.
Источник — DaData `findById/party`: бесплатные 10 000 запросов в сутки на
аккаунт по официальным тарифам на дату проверки. Это общая квота, не лимит
каждого workspace; платная подписка и автоматическая покупка не включаются.

- В авторизованном production UI проверить поиск ИП и обычное сохранение
  реквизитов. Не создавать лишние записи ради повторной проверки поиска.
- Проверить заполненные поля и смену ИНН/сессии/workspace/прав: старый результат
  не должен применяться, заполненные поля не перезаписываются молча, компании
  не объединяются автоматически. Checksum ИНН ограничивает lookup, а не ручной ввод.
- Проверить тёмную форму компании, v2 JSON/CSV с реквизитами и сохранность
  прежнего v1/ручного ввода. Квота или ошибка поставщика не должна блокировать
  ручное создание; сбой проверять изолированно, без отключения рабочего ключа.
- Дополнить обе PDF-инструкции реальным сценарием поиска, подтверждения,
  сохранения и ручного ввода при недоступности поставщика.

### P1 — завершить проверку часов звонков после выпуска

Проверить после релиза сохранение, местные часы, интервал через полночь,
предупреждение/отмену звонка, повторную проверку доступа и текущих данных перед
`tel:`. По телефону или ИНН пояс автоматически не определять.
...
Осталось подтвердить реальные сообщения и отмену устаревших напоминаний
на согласованных подтверждённых каналах. Наличие healthy Sales reminders,
ND reader и broker ACL не является доказательством внешней доставки.
Правила пользователей автоматически не включать. При следующем code-only
обновлении сохранить активный reminder process, его env и прежние ND kinds;
повтор initial provisioning, scope `all` и purge не являются recovery.
...
MVP, а не только двух приоритетных. Завершить оставшиеся production-проверки
на общей основе задач, напоминаний и четырёх CRM-сервисов:
...
- контроль необработанных входящих: настраиваемый срок взятия в работу,
  ...
  продолжать создавать просроченные напоминания. Проверить авторизованный UI,
  реальную доставку и отмену на согласованных подтверждённых каналах.
  Сохранять действующие private ingress, exact SLA/ND queue ACL и два
  process roles того же Intake сервиса; initial activation не повторять.
  `BREACHED` не означает отправку; UI и producer остаются fail-closed до
  actual ND readiness. Source-флаг по умолчанию выключен;
...
- завершить оставшиеся production-проверки Sales-owned центра уведомлений,
  включая авторизованный desktop/mobile UI smoke. Подтвердить атомарное
  создание, dueAt-видимость, прочитано/не прочитано, перенос срока,
  завершение/повторное открытие и current assignment binding/epoch.
  In-app DUE не зависит от email/Telegram или внешних правил тихих часов;
  историю не импортировать. Intake SLA остаётся отдельным owner-контрактом;
- проверить в авторизованном production UI ежедневные/еженедельные/месячные
  серии: срок, ответственный, часовой пояс, пауза/отмена и изменение серии.
  Отдельно подтвердить restart/catch-up без повторного создания задачи;
  каждый период сохраняет уникальный ключ. Не добавлять спринты, эпики
  или Jira-конструктор.
...
### P1 — точная membership-привязка задач из прежних путей создания

`SalesService.createTask` и `IntakeOperationService.execute` создают задачу
с `assignedToSubject`, но без `assignedToMembershipId`. Затронуты
`POST /api/v1/crm/sales/deals`, `POST /api/v1/crm/sales/deals/:id/transition`,
`POST /api/v1/crm/sales/tasks/:id/complete` при создании следующего действия
и `POST /internal/v1/crm-sales/intake-operations/execute` при конвертации входящего.
Для non-OWNER личные правила и получатель ASSIGNEE не отправляют уведомления:
текущая Access authority допускает null membership только подтверждённому OWNER.
Такие записи также не показываются в личном in-app центре до точного назначения.

До обещания уведомлений для этих путей получать свежую точную membership-привязку
через Access HTTP proof; для Intake согласованно расширить bridge-контракт,
сохранив idempotency, tenant/scope и текущие назначения. Не подставлять первую
найденную запись, не считать null «любым текущим сотрудником», не читать чужую БД
и не массово менять исторические задачи. Пока безопасный явный шаг — назначить
задаче ответственного через Планировщик: он сохраняет подтверждённый binding.
Инструкции не должны обещать внешние уведомления для legacy non-OWNER задач
до выпуска этого исправления.
...
### P1 — завершить проверку формы расписания CRM backups и инструкции

- В production-админке проверить обе роли ADMIN/DEV, предпросмотр и сохранение
  12 CRM-полей расписания. Сохранение формы не должно сбрасывать девять прежних
  целей. Не создавать дополнительные задания или dumps ради UI smoke.
- Дополнить администраторскую PDF-инструкцию: четыре независимые CRM-цели,
  проверка статуса штатного задания и доставки архива/`.provenance.json`.
  Подпись — не второй dump и не шифрование; наличие CRM sidecar не разрешает
  destructive restore. Семь прежних restore-целей и release gate не расширять.
- Проверить оставшиеся граничные сценарии расписания: уникальные ключи
  периодов, московскую полночь и ограниченный catch-up без изменения прежних
  целей. Crash/restart и recovery относятся к отдельному P1 ниже; успешная
  штатная доставка сама по себе не доказывает эти сценарии.
...
### P1 — подтвердить production-доставку писем Identity

После восстановления доступности backend VPS сопоставить конкретные жалобы
с безопасными логами `identity_email_delivery` и с участием пользователя
проверить доставку на согласованные ящики, включая спам и задержки. Пройти
регистрацию, повторную отправку, привязку email и восстановление пароля
на согласованном тестовом аккаунте. Проверить принятие старого кода после
неудачной повторной отправки и сохранение прежнего пароля/сессий до входа
с полученным временным паролем.

Публичный DNS, SMTP-авторизация, локальные synthetic SMTP/PostgreSQL/DOM
проверки и health production-контейнера не доказывают получения письма.
Принятие SMTP (`ACCEPTED`) не считать доставкой во входящие. Не отправлять
письма и не восстанавливать реальные аккаунты без согласованных получателей;
не сохранять адреса, коды, пароли, credentials или текст письма в диагностике.
При проблемах доставки сначала сопоставить попытку по `deliveryAttemptId`
с результатом SMTP и почтовым провайдером, не переносить flow в RabbitMQ
и не добавлять автоматические повторы с неизвестным результатом.
...
На фактических viewport 320/390/768 проверить платежи, рассылки, контент,
пользователей и операционные разделы; настройки CRM — на 320/768. Проверить
мобильную колонку, перенос длинных ID/адресов, собственный scroll таблиц
и отсутствие горизонтального overflow страницы. На всех трёх ширинах, включая
настройки CRM, проверить modal/drawer: доступность заголовка/закрытия и
внутренний scroll; отдельно проверить ADMIN без DEV.

Проверять фактический viewport браузера или реальное устройство:
desktop screenshot не заменяет мобильную проверку каждого раздела.
...
cookies и публичные страницы виджетов. Не удалять действующий монорепозиторий
`winwidget.ru_frontends`, его локальные настройки или сохранённую историю.
...
### P1 — ограничить накопление кеша сборки на frontend VPS

Повторные frontend releases могут останавливаться до cutover на обязательном
disk gate 8 GiB: неиспользуемый BuildKit cache накапливается быстрее, чем
освобождается штатной сборкой. Ручная очистка кеша устраняет текущую нехватку
места, но не задаёт постоянную политику обслуживания.

Перед автоматизацией определить бюджет диска, возраст и резерв сохраняемого
кеша, порог оповещения и владельца обслуживания. Очистку выполнять под общим
frontend/Nginx lock, только для неиспользуемого кеша согласованного builder;
сохранять контейнеры, образы отката, volumes, networks, release receipts и
append-only static assets. Обязательны инвентаризация и проверка неизменности
runtime до/после, достаточный запас для последовательной сборки четырёх
приложений и сохранение fail-closed disk gate. Автоматическое удаление старых
релизов требует отдельной согласованной retention-политики.

### P1 — определить retention frontend-релизов и бюджет Docker-сетей

Сети сохранённых frontend-релизов исчерпывают стандартные адресные пулы
Docker. Они остаются связаны с остановленными контейнерами для отката;
нулевое число активных endpoints не означает, что сеть можно удалить.
Следующие выпуски требуют запаса адресов и согласованной политики хранения
прежних релизов.

Определить срок и количество сохраняемых rollback-наборов, условия их
проверенного удаления и порог оповещения о нехватке сетей. Перед очисткой
проверять ссылки всех контейнеров, включая остановленные, release receipts
и текущий production pointer; сохранять рабочий и согласованные rollback-
наборы, данные и опубликованные static assets. Все операции выполнять под
общим frontend/Nginx lock с инвентаризацией до/после. Не добавлять общий
`network prune`, автоматическое удаление старых релизов или изменение
daemon address pools без отдельного согласования.
...
### P2 — стабилизировать конкурентный повтор создания обращения Support

В Services CI `34522665845` от 10.09.2026 два параллельных вызова
`SupportWebService.create` с одинаковыми actor/commandId/payload завершились
ошибкой Prisma write conflict/deadlock в `support-web.integration.spec.ts`.
Код Support в этом SMS-релизе не менялся; production-инцидент не подтверждён.
Serializable-транзакция и уникальный ключ команды защищают от частичного
commit и дублей, но один из конкурентных запросов может завершиться ошибкой.
Текущий retry выполняет четыре попытки без задержки; имеющийся лог не позволяет
отличить исчерпание попыток от непройденной классификации ошибки.

При отдельном согласованном исправлении воспроизвести конфликт с барьером
параллельных транзакций и проверить распознавание кодов Prisma/PostgreSQL,
лимит и интервалы повторов. Закрепить идемпотентный результат для одинаковой
команды, конфликт другого payload и отсутствие дублей сообщений/Outbox.
Диагностика повторов должна содержать только безопасный код ошибки и номер
попытки, без контактов, текста сообщений и credentials. Не маскировать сбой
общим повторным запуском tests или подавлением Prisma errors.
