...
- завершить передачу заявок managed Widgets connector в общую Intake Inbox
  для всех типов виджетов, создающих заявки, и проверить сквозную доставку:
...
- доказать сквозные браузерные сценарии приглашений, CRM-ролей/команд и
  конкурентного применения лимита сотрудников: минимум 2 вместе с владельцем, Trial по умолчанию 5,
  pending и отключённые сотрудники место не занимают;
- завершить paid CRM checkout/продление/дополнительные места и доказать в UI
...
- создать для каждого `crm-*` собственные runtime/migration/backup роли и БД,
  ...
  таблицам;
- подготовить отдельный CRM backend VPS для четырёх сервисов;
...
Новый CRM backend VPS ещё не создан; его деплой отложен. Решение по frontend
...
### P0 — native Widgets Inbox: совместимый reader и сохранение исходных данных

Перед включением передачи заявок добавить отдельный `widget_source_id` с
workspace-bound FK на managed sources. Не записывать его в существующий
`source_id`, принадлежащий API sources. Публичный `sourceId` проецировать по
origin, включая отдельный Prisma select экспортера.

Только `origin=WIDGET` допускает отсутствующее имя. Интерфейс показывает
«Имя не передано», не сохраняет выдуманное имя и запрашивает подтверждённое
имя при создании нового контакта. Оно входит в immutable acceptance payload
и request hash; исходная заявка/snapshot не переписываются. Выбор уже
существующего контакта не должен переименовывать его.

Типизированный snapshot до 256 KiB хранить у Intake и выдавать отдельным
read-only endpoint деталей с текущей CRM-авторизацией; не включать его в
каждую строку paginated list или текущий 20-колоночный экспорт. После доставки
не обращаться к Widgets/Billing для чтения: отключение связи и окончание
Widgets-подписки не скрывают уже полученные CRM-данные.
...
Основной frontend разрешено выпускать на существующий VPS независимо от
готовности отдельного CRM backend VPS. До продвижения CRM frontend-изменений в его `prod` явно отключить
...
### P2 — frontend-монорепозиторий с четырьмя независимыми приложениями
...
05.09.2026 согласован один frontend-монорепозиторий `winwidget.ru_frontends`,
а не четыре отдельных Git-репозитория: `apps/crm`, `apps/widgets`,
`apps/landing`, `apps/admin-panel`. Перенос существующих frontend-репозиториев
выполнять только после локального MVP; текущая разработка CRM продолжается
в существующих checkout. Общие стабильные UI-компоненты и design tokens
выделять в `packages/ui` и `packages/design-tokens` по фактической необходимости.
...
- При переносе сохранить Git-историю и проверенные исходные ревизии обоих
  frontend, не удалять исходные репозитории до проверенного cutover.
...
- Не считать запуск нескольких контейнеров одного полного Next.js-приложения
  ...
  image, production env, CI/deploy, health/smoke и rollback. Изменение общего
  package проверяется во всех зависящих приложениях; runtime-импорты между
  `apps/*` запрещены. Один репозиторий не связывает деплой четырёх приложений
  в обязательный общий релиз.
- Размещение согласовано: все четыре frontend-приложения, включая WinCRM,
  используют один существующий frontend VPS, отдельные контейнеры и
  независимый деплой. Отдельный CRM frontend VPS не требуется для первого
  выпуска; разнос на разные VPS отложен до подтверждённой потребности
  масштабирования. Перед размещением проверить суммарные CPU/RAM/disk,
  лимиты контейнеров и маршрутизацию reverse proxy: общий VPS остаётся единой
  точкой отказа. Итоговые домены/маршрутизацию лендинга, Widgets и админки
  согласовать перед переносом; `crm.winwidget.ru` уже закреплён за WinCRM.
  Готовность frontend-контейнера не разрешает включать CRM без совместимого
  backend rollout. Решение об отдельном CRM backend VPS не меняется.
