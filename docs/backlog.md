...
- завершить native Widgets release gates за пределами owner-only image proof:
  браузерные сценарии подключения/ошибок/retry, OWN/TEAM и межпространственные
  ограничения, принятие безымянной заявки; затем повторить steady-state smoke
...
включая браузерные маршруты после отключения источника/окончания подписки.
