...
- Пользователь отдельно подтвердил 07.09.2026 самостоятельные задачи без
  ...
  Текущая БД требует ровно одну открытую задачу на открытую сделку, а старый
  endpoint завершения обязательно создаёт следующую. Перед новым workflow
  согласованно изменить эти ограничения: поддержать независимые задачи,
  IN_PROGRESS и завершение без принудительного следующего действия; закрытие
  сделки не затрагивает самостоятельные задачи. Старые команды и receipts
  сохраняют прежнюю семантику; новый контракт не должен опираться на fake deal,
  чужую БД или автоматическое переназначение.
