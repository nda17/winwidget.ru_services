...
### P2 — после запуска WinCRM убрать релизную неготовность из админки

Запрос пользователя от 07.09.2026: отложить до запуска CRM. В разделе оплаты
убрать блок «Внешняя проверка» со статусами «Не проверено внешне» для
автоплатежей, онлайн-кассы и ОФД, а также пояснения/предупреждения о релизной
неготовности в tooltip «Read-only готовность YooKassa». Сейчас они находятся в
`apps/admin-panel/src/screens/admin/ui/payments/AdminPayments.tsx` frontend
монорепозитория. Внешние проверки и незавершённые условия выпуска остаются
в техническом backlog, не дублируются как пользовательская проблема в админке.

Не подменять неизвестное состояние зелёным «Проверено» и не отключать backend
проверки прав/конфигурации или реальные ошибки платежей. Удалить связанные
неиспользуемые стили и тексты; проверить desktop/mobile и доступ ADMIN/DEV.
Не удалять общий AdminTooltip или остальные полезные подсказки.
...
### P2 — после запуска WinCRM унифицировать резиновую ширину админки

Третий запрос пользователя от 07.09.2026: убрать разнобой между узкими
центрированными формами и широкими карточками. Основные окна/блоки всех
разделов должны занимать всю доступную ширину рабочей области с едиными
внешними границами, отступами и адаптивной сеткой. Не ограничивать отдельные
формы случайными `max-width`; при необходимости группировать поля внутри
широкого общего контейнера. Высота — по содержимому, одинаковые карточки
в одном ряду выравниваются, без принудительной одинаковой высоты всех экранов.

Сделать общий layout-паттерн вместо исправлений отдельных вкладок. Для modal/
drawer сохранить безопасные отступы от viewport, доступные заголовок/закрытие
и управляемый внутренний scroll. На 320/390/768/1440 проверить платежи,
настройки CRM, рассылки, контент, пользователей и операционные разделы:
мобильная колонка, перенос длинных ID/адресов, таблицы со своим scroll,
отсутствие горизонтального overflow страницы и наложений элементов.
Стили через `@apply`, удаление остаточных ширин и визуальная browser-проверка.
Backend-поведение и права не меняются.

### P2 — после production-запуска CRM MVP удалить временные папки из корня

Четвёртый запрос пользователя от 07.09.2026: после выпуска CRM MVP убрать
ненужные временные checkout/worktree, release/test-папки и пустые каталоги
из корня workspace. В том числе проверить рабочие копии с суффиксами
`identity_manifest`, `operations_api`, `notes_rollout`, `operations_restore`,
`otp_release`, `otp_rollout`, `worker_recovery`; точные имена определить
инвентаризацией, а не удалять по широкому wildcard.

Перед удалением сверить `git worktree list`, статус каждой копии, уникальные
неотправленные коммиты и незакоммиченные изменения. Нужные изменения сначала
сохранить в основных репозиториях; worktree удалять штатной командой Git.
Сохранить `winwidget.ru_client`, `winwidget.ru_services`, `winwidget.ru_infra`,
актуальные `deploy`/секреты и `DOCUMENTATION`; не читать `TEMP`/`other_files`
в нарушение инструкций проекта. Отдельно проверить активные процессы и ссылки
на удаляемые пути. Не создавать новые backup/dump-копии для этой уборки.
После очистки сообщить точный список удалённого и возможность восстановления.
