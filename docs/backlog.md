...
### P0 — выпустить продуктовую структуру сайта вместе с MVP

Пользователь включил изменение публичного frontend в MVP 06.09.2026:
`winwidget.ru/` — главная экосистемы, `/products/widgets` — прежний полный
лендинг виджетов, `/products/crm` — маркетинговая страница WinCRM.
...
До выпуска проверить:

- совместимый Platform validator и сохранение `ecosystem`/`crmProduct`
  при PATCH из уже открытого старого редактора; новый frontend отправляет
  расширенный документ, поэтому **Platform выпускается раньше frontend**;
- отдельные редакторы главной, Widgets, WinCRM и SEO в «Контенте»;
...
- сохранность текущего контента Widgets, query/UTM и старых точных
  `/#tools`, `/#pricing`, `/#faq` ссылок; canonical/sitemap трёх страниц,
  включая сохранённые `enabled:false` записи sitemap;
...
- desktop/mobile 320/390/768/1440, клавиатуру, контраст, формы редакторов,
  ...
  на публичных маркетинговых страницах;
- правдивые статусы и тексты до открытия CRM; CMS не управляет release gate,
  ценами, URL интеграций или Trial. Открытие продаж и активация Trial
  допускаются только после остальных backend/business gates MVP.

Новый бренд, репозиторий, пятое frontend-приложение или общий backend
для этой структуры не нужны. Новые страницы используют текущий Platform
JSON-контракт без новой БД и миграции. До браузерной проверки, совместимого
rollout и production smoke задачу не считать выпущенной.
...
### P1 — выпустить исправление фильтра ошибок доставки Operations

Production UI `/admin/messaging` передаёт статус `FAILED`, который текущий
Operations runtime отклоняет до обращения к БД и соседним сервисам. Выпустить
проверенное исправление read-фильтра с сохранением legacy `OPEN`/`UNRESOLVED`;
