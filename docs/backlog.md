...
- Завершить отдельную активацию оплаты: три полных owner/canonical env уже
  ...
  Выпустить pinned config-only `crm-commerce-activate` по свежему baseline,
  затем проверить фактические runtime flags, независимость Widgets и отсутствие
  самопроизвольных provider operations. Подготовленные настройки и успешный
  CI не заменяют подтверждение фактической активации.
