...
### P1 — упростить интерфейс резервных копий и восстановления

По запросу пользователя от 06.09.2026 после текущей стабилизации переработать
UX: технические release-gate, rehearsal, worker и read-only объяснения сейчас
смешаны с повседневными действиями и затрудняют выбор безопасного сценария.
Перед реализацией согласовать компактную структуру и матрицу прав отдельно
для просмотра, создания копии и восстановления.

- Разделить обзор состояния, список резервных копий, расписание и мастер
  восстановления; показывать понятные статус, дату последней пригодной копии,
  причину недоступности действия и следующий необходимый шаг.
- Скрыть технические подробности в раскрываемом блоке, не выдавая секреты;
  сохранить доступный ADMIN read-only экран и отдельные DEV-only опасные
  действия, backend guards и журнал событий.
- Не снимать `DATABASE_RESTORE_ENABLED=false` ради упрощения UI. Сохранить
  проверку копии, подписи/целевой БД, rehearsal, writer fence, safety backup,
  подтверждение опасного действия и recovery evidence.
- Проверить desktop/mobile, загрузку/ошибки/частичную недоступность, доступ
  ADMIN/DEV и корректное восстановление статуса незавершённого задания.
...
соседнего приложения. Локальные проверки не заменяют production-сессию.
