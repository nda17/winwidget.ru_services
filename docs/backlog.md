...
- По явному подтверждению пользователя 07.09.2026 включить платную WinCRM
  в текущем frontend-релизе CRM/Widgets после уже выполненной backend-активации,
  сохранив независимость оплат Widgets. Работающие backend-флаги не подтверждают
  внешние списания/фискализацию; реальные проверки ниже остаются с участием
  пользователя. Обычный code-only `crm-upgrade` не разрешает менять env
  или повторять уже завершённую config-only активацию.
...
- В авторизованном production UI проверить явный поиск головной организации
  и ИП, просмотр результата, подтверждение заполнения и обычное сохранение
  реквизитов. Наличие ключа в healthy Customers API не доказывает этот сценарий.
...
- Проверить мобильную/тёмную форму, v2 JSON/CSV с реквизитами и сохранность
...
### P1 — выпуск часового пояса контакта и удобных часов звонков

Выпустить Customers migration `20260907223000_add_contact_call_preferences`
и frontend с v2 contacts, сохранив v1 DTO/receipt replay и ручные записи.
Часовой пояс и обе границы времени необязательны; незаполненные значения
показываются явно. Пояс контакта не связан с поясом фильтра Планировщика.
...
- контроль необработанных входящих: настраиваемый срок взятия в работу,
  ...
  продолжать создавать просроченные напоминания. Завершить production-выпуск
  по `apps/crm-intake/README.md`: PG18 migration/acceptance-trigger gate,
  CI grants/backup inventory, private ingress, exact SLA/ND queue ACL и два
  process roles того же Intake сервиса. Readers Access/Intake/ND до producer,
  затем отдельная scoped activation и проверка реальной доставки/отмены.
...
- завершить production-выпуск Sales-owned центра уведомлений: миграция
  `20260908140000_add_task_notification_center`, exact runtime grants и
  backup inventory, затем API и frontend. Проверить на PG18 атомарное
...
- В production-админке проверить ADMIN/DEV, чтение и предпросмотр 12 CRM-полей
  расписания и 13 backup-целей. Сохранение формы не должно сбрасывать девять
  прежних целей. Не создавать дополнительные задания или dumps ради UI smoke.
