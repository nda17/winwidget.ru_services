...
- До включения Workday writers добавить экспорт самостоятельных задач с
  проверенным OWN/TEAM/ALL и доступностью в READ_ONLY. Текущий Sales export
  `tasks` отбирает только строки со связанной сделкой, а старый frontend reader
  требует non-null dealId. Выпустить совместимый отдельный/версионированный
  контракт и подключить его к «Моему дню»; не выдавать старый частичный экспорт
  за выгрузку всех задач. Сохранить server-side snapshot/pagination, byte/row
  limits, повторную проверку authority и аудит экспорта, не открывать чужие
  задачи/сделки и не переписывать старые receipts.
