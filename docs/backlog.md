...
- доказать на release images передачу заявок managed Widgets connector в общую
  Intake Inbox для всех типов виджетов, создающих заявки, и сквозную доставку:
...
Перед включением доказать на точных образах всех шести типов виджетов
сохранение исходного snapshot, выдачу деталей без обращения к Widgets после
отключения источника/окончания подписки и отдельную проверку доступа OWN/TEAM.
Проверить создание контакта с подтверждённым именем для безымянной WIDGET-заявки
и отсутствие переименования выбранного существующего контакта.

### P0 — безопасный rollout отложенных retry новых CRM workflows

Перед выпуском database-delayed retry widget-control и acceptance доказать
outage, restart и повторную доставку после commit/ack на точных образах.
...
проверить на их будущем backend VPS до открытия рабочего продукта.
...
### P1 — выпуск резервного входа при недоступности CAPTCHA

До включения `IDENTITY_LOGIN_OTP_ENABLED=true` выпустить миграцию
`20260910010000_add_login_otp` и совместимый Identity runtime, проверить
service-owned runtime/maintenance ACL двух `login_otp_*` таблиц и актуальный
restore manifest. Синхронизировать production env в обе стороны, не включать
сценарий только клиентским признаком недоступности CAPTCHA. Существующий
Gateway prefix `/api/v1/auth` уже ведёт в Identity; проверить точные методы
`GET /login-otp/capabilities`, `POST /login-otp/request`,
`POST /login-otp/verify` под этим prefix, без отдельного catch-all.

Перед включением проверить эффективный trusted-proxy boundary и независимые
лимиты IP/контакта/канала в PostgreSQL. Отправка допускается только для
подтверждённого контакта активного существующего пользователя, а одинаковый
ответ не подтверждает доставку сообщения. Проверить фактическую готовность
SMTP/SMS; реальные отправки тестовых кодов требуют участия пользователя.
Флаг по умолчанию выключен, обычный вход и регистрация сохраняют CAPTCHA.
Frontend должен отдельно обрабатывать неготовый backend без фиктивной
...
### P1 — production cutover четырёх frontend-приложений

До переключения текущего frontend VPS на `winwidget.ru_frontends` закончить
браузерную проверку desktop/mobile отдельно для `apps/landing`, `apps/widgets`,
`apps/admin-panel` и `apps/crm`: вход, профиль с отдельным статусом WinCRM,
переключатель оплаты, новая навигация админки, переходы между продуктами,
редактирование контента и недоступность соседнего приложения.

Выпускать только exact green-CI SHA с проверенными четырьмя immutable images,
trusted SSH host key, byte-identical production env и закреплённым Nginx
artifact. Перед cutover повторить capacity/port gates, выпустить отдельный
сертификат `crm.winwidget.ru` без изменения основного сертификата и сохранить
точный rollback прежних контейнера, образа и конфигурации. Инструкция и
контроллер выпуска находятся в frontend-репозитории и `winwidget.ru_infra`.
...
cookies и публичные страницы виджетов. Исходный CRM-репозиторий не удалять до
проверенного cutover; переименование физических checkout согласовать с
активными локальными процессами и CI/CD paths. Не путать готовность frontend
с выпуском четырёх CRM backend-сервисов: они пока не деплоятся, CRM-ссылка
