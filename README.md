...
Права CRM-БД принадлежат сервисам: `apps/crm-*/prisma/database-access.json`
фиксирует точные таблицы, последовательности, enum и trigger routines.
`deploy/crm/database-access.mjs` читает эти manifests и SHA-256 миграций,
формирует транзакционный bootstrap отдельных migration/runtime/backup ролей
и точные post-migration grants. Модуль допускает только PostgreSQL 18,
ожидаемого владельца и отдельный кластер без чужих БД/ролей/схем.
Runtime не получает DDL/TEMP, права на журнал миграций или изменение
append-only истории. Повторный bootstrap не меняет существующие пароли.
Незавершённые/изменённые миграции, лишние объекты, column grants и
расширенные роли останавливают применение. Backup-роль только читает;
этот модуль не запускает задания и не создаёт дополнительные копии данных.

Это SQL-компонент, не самостоятельный production controller: общая блокировка
деплоя, проверка exact image/env/source и реальная аутентификация всех ролей
обязательны у вызывающего controller. Результат bootstrap содержит секреты:
передавать только в stdin `psql -X` с `ON_ERROR_STOP`, без вывода/артефактов
и без объединения всего скрипта в один query API. Первые отдельные `SET`
отключают SQL-логирование до отправки команд с паролями.

Локальная проверка после установки зависимостей четырёх CRM-приложений
требует работающую **локальную Colima**. Она создаёт четыре независимых
одноразовых PostgreSQL 18 последовательно, применяет реальные Prisma
миграции, проверяет SCRAM-аутентификацию, права, повторный запуск и отказ
при drift. Production URL/существующие контейнеры не принимаются.

```bash
node --test deploy/crm/database-access.test.mjs
node deploy/crm/database-access.integration.mjs --local all
```

В CI те же проверки выполняются для каждого владельца отдельно и входят
в зависимости production job. Тест удаляет свои контейнеры/volumes;
после локальных Docker-проверок дополнительно выполнить согласованную
очистку images/build cache/осиротевших anonymous volumes и остановить Colima.
Эта проверка не подтверждает ресурсный запас под одновременной нагрузкой
12 CRM-процессов и не открывает публичный release gate.
