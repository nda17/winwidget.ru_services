...
function setup() {
	...
	return {
		...
		audit,
		findMany,
		service: new IntakeExportService(
			prisma as unknown as CrmIntakePrismaService,
			{ authorize } as unknown as IntakeAuthorizationClient
		)
	};
}
describe('Intake export snapshot authorization', () => {
	...
	test.each([
		...
		await expect(
			service.prepare('Bearer user', workspaceId, 'inbox', 'json')
