...
import { IntakeExportController } from './export.controller';
...
describe('Intake export actual HTTP contract', () => {
	...
	beforeAll(async () => {
		const module = await Test.createTestingModule({
			controllers: [IntakeExportController],
			providers: [{ provide: IntakeExportService, useValue: { prepare } }]
		}).compile();
	...
	) => origin + '/api/v1/crm/intake/exports/' + entity + '?' + query;
	test('sends whole attachment with exact logical byte metadata and CORS exposure', async () => {
		...
		expect(response.headers.get('content-disposition')).toBe(
			'attachment; filename="wincrm-inbox.json"'
		);
	...
	test.each([
		...
		const response = await fetch(url('inbox', query), {
