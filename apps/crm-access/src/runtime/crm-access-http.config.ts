...
export function getCrmAccessTrustProxyConfig(
...
export function getCrmAccessCorsAllowedOrigins(
...
export function getCrmAccessListenHost(
	mode: string | undefined,
	value: string | undefined
): string {
	...
	if (['0.0.0.0', '::', '*'].includes(host)) {
		throw new Error(
			'CRM_ACCESS_LISTEN_HOST must not be a wildcard address'
		);
	}
	if (
		mode !== 'development' &&
		...
		throw new Error(
			'CRM_ACCESS_LISTEN_HOST must be loopback in production'
		);
	}
	if (!/^[A-Za-z0-9.:-]{1,255}$/.test(host)) {
		throw new Error('CRM_ACCESS_LISTEN_HOST is invalid');
	}
	return host;
}
