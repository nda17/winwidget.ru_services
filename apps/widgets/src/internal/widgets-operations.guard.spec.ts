...
const TOKEN = 'widgets-operations-test-token-at-least-32-characters';
...
describe('WidgetsOperationsGuard', () => {
	const guard = (value: string | undefined) =>
		new WidgetsOperationsGuard({
			get: () => value
	...
	it.each([undefined, 'short', 'change_me'])(
		...
		value => {
			expect(() => guard(value)).toThrow('WIDGETS_OPERATIONS_TOKEN');
		}
	);
});
