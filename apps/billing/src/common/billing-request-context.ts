...
const storage = new AsyncLocalStorage<BillingRequestContext>();

export function billingRequestContextMiddleware(
	request: Request,
	response: Response,
	next: NextFunction
): void {
...
export function getBillingCorrelationId(): string {
	return storage.getStore()?.correlationId || randomUUID();
}

export function getBillingClientContext(request: Request): {
	ip?: string;
