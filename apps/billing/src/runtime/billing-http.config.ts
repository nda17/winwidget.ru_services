...
export function getBillingTrustProxyConfig(
...
export function getBillingCorsAllowedOrigins(
...
export function getBillingListenHost(
	mode: string | undefined,
	value: string | undefined
): string {
	const host = value?.trim() || '127.0.0.1';
	if (['0.0.0.0', '::', '*'].includes(host)) {
		throw new Error('BILLING_LISTEN_HOST must not be a wildcard address');
	}
	if (
		mode !== 'development' &&
		!['127.0.0.1', '::1', 'localhost'].includes(host)
	) {
		throw new Error('BILLING_LISTEN_HOST must be loopback in production');
	}
	if (!/^[A-Za-z0-9.:-]{1,255}$/.test(host)) {
		throw new Error('BILLING_LISTEN_HOST is invalid');
	}
	return host;
}
