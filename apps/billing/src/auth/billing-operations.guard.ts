...
const PLACEHOLDERS = new Set([
	...
	'ci_billing_operations_token_at_least_32_chars'
]);

@Injectable()
export class BillingOperationsGuard implements CanActivate {
	private readonly token: Buffer;

	constructor(config: ConfigService) {
		const value =
			config.get<string>('BILLING_OPERATIONS_TOKEN')?.trim() || '';
		if (value.length < 32 || PLACEHOLDERS.has(value)) {
			throw new Error(
				'BILLING_OPERATIONS_TOKEN must be a non-placeholder secret with at least 32 characters'
			);
		}
