...
@Injectable()
export class BillingCrmAccessGuard implements CanActivate {
	private readonly token: Buffer;

	constructor(config: ConfigService) {
		const value =
			config.get<string>('BILLING_CRM_ACCESS_TOKEN')?.trim() || '';
		if (value.length < 32 || PLACEHOLDERS.has(value)) {
			throw new Error(
				'BILLING_CRM_ACCESS_TOKEN must be a non-placeholder secret with at least 32 characters'
			);
		}
