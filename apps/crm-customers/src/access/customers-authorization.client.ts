...
export interface CustomersAuthorization {
	schemaVersion: 1;
	workspaceId: string;
	subject: string;
...
export function parseCustomersAuthorization(
	value: unknown,
	workspaceId: string
): CustomersAuthorization | null {
	...
	if (
		...
		return null;
	return data as unknown as CustomersAuthorization;
}

export function parseCustomersAccessOrigin(
...
export function assertCustomersPermission(
	...
	if (
		...
		throw new ForbiddenException({
			...
			message: 'Customer action is not allowed'
		});
	}
}

@Injectable()
export class CustomersAuthorizationClient {
	...
	async authorize(
		...
		try {
			...
			const context = parseCustomersAuthorization(
			...
			throw new ServiceUnavailableException({
				code: 'crm_customers_access_unavailable',
