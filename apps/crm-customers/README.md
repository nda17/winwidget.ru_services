...
```bash
node .github/scripts/test-crm-bootstrap-failure.mjs crm-customers
```
...
### Часовой пояс и удобное время звонка контакту v2

`/api/v1/crm/customers/v2/contacts` сохраняет прежние CRUD, duplicates,
activities, права и CAS, с `schemaVersion: 2` в командах и ответах.
Карточка дополнительно всегда содержит nullable `timeZone` (IANA, до 100
символов), `preferredCallStart` и `preferredCallEnd` (`HH:mm`). Можно задать
только часовой пояс. Интервал требует оба разных времени и часовой пояс;
границы трактуются как `[start,end)` по местному времени контакта, переход
через полночь разрешён. Это предпочтение для звонков, а не расписание отправки
уведомлений: настройки Planner независимы, автоматического определения по
телефону или другим данным нет.

Пропущенные новые поля PUT сохраняются; явный `null` очищает поле. При
удалении часового пояса нужно также очистить оба времени имеющегося интервала.
Некорректная итоговая комбинация возвращает `400 crm_contact_call_preferences_invalid`.
JSON/CSV `/api/v1/crm/customers/exports/v2/contacts` включает все три поля,
с прежними правами экспорта и лимитами. V1 JSON, хэши и replay команд,
экспорт v1 и Intake не меняются; старый клиент не стирает новые настройки.
Миграция `20260907223000_add_contact_call_preferences` добавляет nullable
колонки без backfill, существующие контакты остаются без этих предпочтений.

### Реквизиты компаний v2 и поиск по ИНН

Расширенные карточки используют `/api/v1/crm/customers/v2/companies`:
GET list/detail/activities, POST create/archive и PUT update с прежними
workspace/scope/CAS-проверками. Тело команды и envelope ответа имеют
`schemaVersion: 2`; `Idempotency-Key` по-прежнему равен `commandId`.
Добавлены nullable `legalName` (до 2000), `kpp` (9 цифр), `ogrn` (13/15 цифр),
`legalAddress` (до 2000), `entityType` (`LEGAL|INDIVIDUAL`).
Пропущенные **новые** поля PUT сохраняют прежнее значение; явный `null`
очищает поле. Семантика прежних редактируемых полей не меняется.

V1 routes, response fields и хэши прежних команд сохранены. Старый клиент
может обновить компанию, не стирая её реквизиты. Миграция
`20260907210000_add_company_requisites` добавляет только nullable-колонки и
ограничения новых полей; backfill, слияние по ИНН и изменение контактов
не выполняются. Readiness проверяет все новые колонки.

`POST /api/v1/crm/customers/company-lookup` принимает
`{schemaVersion:1,workspaceId,inn}`. ИНН — ровно 10 цифр для юридического лица
или 12 для ИП, с проверкой контрольных цифр. Это более строгая проверка
**только поиска**, не изменение правил ручного сохранения ИНН.
Текущие `customers:read`, `customers:write`, `ACTIVE|GRACE` нужны до запроса
и перед выдачей результата. Смена субъекта, роли, scope, команды или прав
отклоняет результат. В READ_ONLY доступен просмотр сохранённых реквизитов,
но не внешний поиск для автозаполнения.

Ответ поиска: `{schemaVersion:1,provider,queriedAt,inn,items}`. `provider` —
стабильный код источника (`^[A-Z][A-Z0-9_]{1,31}$`), сейчас `DADATA`.
До пяти результатов содержат `name`, `legalName`, `inn`, `kpp`, `ogrn`,
`legalAddress`, `entityType`, `status`. Результат — подсказка для черновика,
не подтверждение сохранения и не юридическое заключение о контрагенте.
`queriedAt` означает время получения ответа, а не свежесть государственного
реестра. Для ИП доступен адрес в объёме поставщика, иногда только город;
полный адрес не угадывается. Неработающий бизнес не скрывается: UI показывает
предупреждение, пользователь сам решает, сохранять ли карточку.
У ИП DaData может не возвращать КПП и признак филиала: адаптер нормализует
КПП в `null`, не требует отсутствующего признака и по-прежнему отклоняет
явный `BRANCH`. Несколько регистраций одного ИНН остаются отдельными
вариантами выбора; автоматически объединять их нельзя.

Поставщик — [DaData findById/party](https://dadata.ru/api/find-party/).
Он подключён через серверный DI-порт `CompanyLookupProvider` и отдельный
`DadataCompanyLookupAdapter`. Только адаптер знает ключ, протокол и структуру
ответа DaData; общая служба сохраняет авторизацию, кэш и ограничения частоты.
Новый источник реализует этот порт и заменяет binding в `CrmCustomersModule`,
возвращая тот же нормализованный контракт. Frontend не ограничен `DADATA`:
неизвестный допустимый код отображается как «Источник реквизитов».
Карточки компаний и формы не зависят от поставщика; автоматического fallback
на второй платный сервис, нового runtime или переключения одной заменой ключа нет.
Используется только фиксированный HTTPS POST с `branch_type: MAIN`, типом
организации и `count: 5`, без произвольных URL и redirect. Тайм-аут — 5 секунд,
ответ — максимум 2 MiB; внешние ошибки/сырые данные не логируются.
`CRM_CUSTOMERS_DADATA_API_KEY` — optional server-only API-ключ; отдельный
DaData secret key и платный API «Стандартизация» не используются. Отсутствие
ключа не блокирует запуск сервиса и ручной CRUD. Ключ должен получать только
Customers API, не frontend, migration, другие CRM-сервисы или RabbitMQ.

Согласно [тарифам DaData](https://dadata.ru/pricing/), проверенным 07.09.2026,
бесплатная квота — 10 000 запросов/сутки **на аккаунт поставщика**, суммарно
с другими его сервисами подсказок. Начальная платная подписка — 14 000 руб./год
за 50 000 запросов/сутки; интеграция не оформляет и не оплачивает подписку.
Кэш только публичных реквизитов: 15 минут, пустой ответ — 30 секунд, до 1000
ключей. Single-flight объединяет одновременный поиск одного ИНН. Защита одного
процесса — 30 запросов/минуту суммарно, 20/workspace, 10/subject в workspace,
максимум два внешних запроса одновременно. Это не распределённая квота;
масштабирование replicas требует пересмотра общего бюджета поставщика.

Пустой `items` означает корректный ответ без совпадений. Невалидный ИНН — 400;
локальный лимит — 429 с `retryAfterSeconds`; отсутствие ключа, лимит/ошибка
DaData или повреждённый ответ — безопасный 503. Ручное заполнение остаётся
доступным во всех этих случаях. Поиск не создаёт компанию, audit или command
receipt; обычное подтверждённое сохранение атомарно пишет карточку, activity
с именами изменённых полей и receipt. Заполненные поля меняются только после
