...
DO $$
BEGIN
    IF to_regnamespace('crm_customers') IS NULL THEN
        EXECUTE 'CREATE SCHEMA "crm_customers"';
    END IF;
END
$$;
...
CREATE TABLE "crm_customers"."service_identity" (
    ...
    CONSTRAINT "service_identity_name_check" CHECK ("service_name" = 'crm-customers-service')
);

CREATE UNIQUE INDEX "service_identity_database_id_key"
ON "crm_customers"."service_identity"("database_id");

INSERT INTO "crm_customers"."service_identity" (
    "id",
    "service_name",
    "database_id",
    "created_at",
