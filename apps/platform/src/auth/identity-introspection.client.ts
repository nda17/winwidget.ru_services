...
import type { PlatformActor } from './platform-request';
...
const PLACEHOLDER_TOKENS = new Set([
	...
	'change_me_platform_identity_token_at_least_32_chars',
...
export class IdentityIntrospectionClient {
	...
	constructor(config: ConfigService, runtime: PlatformRuntimeService) {
		...
		this.token =
			config.get<string>('IDENTITY_PLATFORM_TOKEN')?.trim() || '';
		if (
			runtime.apiEnabled &&
			(this.token.length < 32 || PLACEHOLDER_TOKENS.has(this.token))
		) {
			throw new Error(
				'IDENTITY_PLATFORM_TOKEN must be a non-placeholder secret with at least 32 characters'
			);
		}
	...
	async introspect(authorization: string): Promise<PlatformActor> {
		...
		if (response.status === 403) {
			throw new ServiceUnavailableException(
				'Authorization service rejected its Platform credential'
			);
		}
		if (!response.ok) {
			throw new ServiceUnavailableException(
	...
	private isActor(value: unknown): value is PlatformActor {
		if (!value || typeof value !== 'object' || Array.isArray(value)) {
			return false;
		}
